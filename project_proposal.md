...
- [x] **圖像生成串接**：成功串接 RunPod A1111 API，支援 Prompt 組合與 LoRA 掛載。
- [x] **角色管理**：支援《原神》、《崩壞：星穹鐵道》及原創角色數據導入。
- [x] **記憶與日記**：實作了基於總結的記憶系統與每日日記生成。
- [x] **UI 優化**：完成主介面、設定頁面與角色詳情頁的視覺設計。

## 5. 未來規劃 (Roadmap)

### Phase 1: 穩定與優化 (Current)
- 優化圖像生成的 Prompt 工程，提高手指與細節的正確率。
- 調整 RunPod Cold Start 的等待體驗 (加入佇列提示)。

### Phase 2: 聲音與多模態 (Next)
- **語音合成 (TTS)**：串接 VITS 或 ElevenLabs，讓角色能發出聲音。
- **Live2D / 動態立繪**：嘗試讓生成的圖片具備簡單的呼吸或眨眼效果。
